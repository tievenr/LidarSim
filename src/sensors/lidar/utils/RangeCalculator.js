...
export class RangeCalculator {
     ...
     */
    static calculateMaxRange(materialReflectivity) {
        if (materialReflectivity <= LIDAR_SPECS.LOW_REFLECTIVITY) {
            return LIDAR_SPECS.MAX_RANGE_LOW_REFLECTIVITY;
        } 
        
        if (materialReflectivity >= LIDAR_SPECS.HIGH_REFLECTIVITY) {
            return LIDAR_SPECS.MAX_RANGE_HIGH_REFLECTIVITY;
        }

        // Linear interpolation between low and high reflectivity ranges
        const rangeDiff = LIDAR_SPECS.MAX_RANGE_HIGH_REFLECTIVITY - LIDAR_SPECS.MAX_RANGE_LOW_REFLECTIVITY;
        const reflectivityFactor = (materialReflectivity - LIDAR_SPECS.LOW_REFLECTIVITY) / 
                                 (LIDAR_SPECS.HIGH_REFLECTIVITY - LIDAR_SPECS.LOW_REFLECTIVITY);
        return LIDAR_SPECS.MAX_RANGE_LOW_REFLECTIVITY + (rangeDiff * reflectivityFactor);
    }
     ...
     */
    static isInRange(distance, materialReflectivity) {
        // Check blind zone
        if (distance < LIDAR_SPECS.BLIND_ZONE) {
            return false;
        }

        // Check maximum range based on reflectivity
        const maxRange = this.calculateMaxRange(materialReflectivity);
